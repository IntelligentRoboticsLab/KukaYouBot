...
class YouBot {
    ...
    ros::Subscriber subscriber_voice_;
...
public:
    ...
    YouBot() {
        ...
        subscriber_voice_ = n_.subscribe( "/transcoding", 1, &YouBot::performAction, this );
    ...
    void performAction( const std_msgs::String &msg ) {

        double latSpeed = 0.03;
        double rotSpeed = 0.08;

        if( msg.data == "rotate right" )    this->setBaseSpeed( 0, 0, -rotSpeed );
        if( msg.data == "rotate left" )     this->setBaseSpeed( 0, 0, rotSpeed );
        if( msg.data == "right" )           this->setBaseSpeed( 0, -latSpeed, 0 );
        if( msg.data == "left" )            this->setBaseSpeed( 0, latSpeed, 0 );
        if( msg.data == "forward" )         this->setBaseSpeed( latSpeed, 0, 0 );
        if( msg.data == "backward" )        this->setBaseSpeed( -latSpeed, 0, 0 );
        if( msg.data == "stop" )            this->setBaseSpeed( 0, 0, 0 );
